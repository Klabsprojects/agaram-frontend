...
@Component({
  selector: 'app-utilityforms',
  templateUrl: './utilityforms.component.html',
  styleUrl: './utilityforms.component.css'
})
export class UtilityformsComponent {
  ...
  ngOnInit(): void {
    ...
    this.actrulesForm = this.fb.group({
      UtilityNumber: ['', Validators.required],
      UtilityDate: ['', Validators.required],
      UtilityDescription: ['', Validators.required],
      UtilityFile: [null, Validators.required]
    });
  }
  ...
  onFileSelected(event: any) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      this.actrulesForm.patchValue({ orderFile: this.selectedFile });
    }
    this.selectedFile = event.target.files[0];
    this.actrulesForm.get('UtilityFile')?.setValue(this.selectedFile);
    if (this.selectedFile) {
      if (this.selectedFile.type !== 'application/pdf') {
        this.actrulesForm.get('UtilityFile')?.setErrors({ 'incorrectFileType': true });
        return;
      }

      if (this.selectedFile.size > 5 * 1024 * 1024) { // 5MB in bytes
        this.actrulesForm.get('UtilityFile')?.setErrors({ 'maxSize': true });
        return;
      }

      this.actrulesForm.get('UtilityFile')?.setErrors(null);
    }
  }
  getlist() {
    this.foreignVisitService.uploadGet('getUtilityforms').subscribe((res: any) => {
  ...
  create() {
    ...
    const fileControl = this.actrulesForm.get('UtilityFile');
  ...
  edit(data: any) {
    ...
    this.actrulesForm.get('UtilityNumber')?.setValue(data.UtilityNumber);
    this.actrulesForm.get('UtilityDate')?.setValue(data.UtilityDate);
    this.actrulesForm.get('UtilityDescription')?.setValue(data.UtilityDescription);
    this.actrulesForm.get('UtilityFile')?.setValue(data.UtilityFile);
  }
  delete(id: any) {
    this.foreignVisitService.uploadDelete('deleteUtilityforms', id).subscribe((res: any) => {
  ...
  onSubmit() {
    ...
    if (this.actrulesForm.valid) {
      ...
      for (const key in formValues) {
        if (formValues.hasOwnProperty(key) && key !== 'UtilityFile') {
          formData.append(key, formValues[key]);
        }
      }
      if (this.selectedFile) {
        formData.append('UtilityFile', this.selectedFile);
      }
      if (this.isEdit && this.editId) {
        formData.append('id', this.editId.toString());
        this.foreignVisitService.uploadEdit(formData, 'updateUtilityforms').subscribe((res: any) => {
          if (res.status === 200) {
        ...
        this.foreignVisitService.uploadAdd(formData, 'addUtilityforms').subscribe((res: any) => {
          console.log("res", res);
          if (res.status === 200) {
