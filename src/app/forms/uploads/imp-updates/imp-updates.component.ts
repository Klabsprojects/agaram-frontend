...
@Component({
  ...
  styleUrl: './imp-updates.component.css'
})
export class ImpUpdatesComponent {
  ...
  ngOnInit(): void {
    ...
    this.actrulesForm = this.fb.group({
      ...
      announcementText: ['', Validators.required],
    });
  }
  ...
  getlist() {
    this.foreignVisitService.uploadGet('getRecentAnnouncements').subscribe((res: any) => {
  ...
  onSubmit() {
    ...
    if (this.actrulesForm.valid) {
      ...
      if (this.isEdit && this.editId) {
        ...
        this.foreignVisitService.uploadAdd(formValues, 'addAnnouncement').subscribe((res: any) => {
          if (res.status === 200) {
