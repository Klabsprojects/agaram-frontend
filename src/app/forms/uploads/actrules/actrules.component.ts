...
@Component({
  selector: 'app-actrules',
  templateUrl: './actrules.component.html',
  styleUrl: './actrules.component.css'
})
export class ActrulesComponent {
  ...
  ngOnInit(): void {
    ...
    this.actrulesForm = this.fb.group({
      actsrulesNumber: ['', Validators.required],
      actsrulesDate: ['', Validators.required],
      actsrulesDescription: ['', Validators.required],
      actsrulesFile: [null, Validators.required]
    });
  }
  ...
  getlist() {
    this.foreignVisitService.uploadGet('getActsrules').subscribe((res: any) => {
  ...
  edit(data: any) {
    ...
    this.actrulesForm.get('actsrulesNumber')?.setValue(data.actsrulesNumber);
    this.actrulesForm.get('actsrulesDate')?.setValue(data.actsrulesDate);
    this.actrulesForm.get('actsrulesDescription')?.setValue(data.actsrulesDescription);
    this.actrulesForm.get('actsrulesFile')?.setValue(data.actsrulesFile);
  }
  view(data:any){
  ...
  download(data: any) {
    const pdfUrl = `${this.url}${data}`;
    // const pdfUrl = `http://127.0.0.1:5000/download/dummy.pdf`
    fetch(pdfUrl)
      .then(response => {
        if (!response.ok) {
  ...
  onSubmit() {
    ...
    if (this.actrulesForm.valid) {
      ...
      for (const key in formValues) {
        if (formValues.hasOwnProperty(key) && key !== 'actsrulesFile') {
          formData.append(key, formValues[key]);
        }
      }
      if (this.selectedFile) {
        formData.append('actsrulesFile', this.selectedFile);
      }
      if (this.isEdit && this.editId) {
        ...
        this.foreignVisitService.uploadAdd(formData, 'addActsrules').subscribe((res: any) => {
          console.log("res", res);
          if (res.status === 200) {
